...
public class VoronoidController
{
    ...
    public void InitVoronoid(int grapfMaxWidth,int grapfMaxHeight,int maxMineCount,Grapf<Node<Vector2Int>> grapf)
    {
        ...
        {
            mines.Add(new MinesHolder(new Vector3(node.GetCoordinate().x,node.GetCoordinate().y), new Poligon(vertex)));
        }
    ...
    public void OnDrawGizmos()
    {
        ...
        for (int j = 0; j < mines.Count; j++)
        {
            for (int i = 0; i < mines[j].poligon.vertices.Count; i++)
            {
                ...
                Vector3 vertex2 = new Vector3(mines[j].poligon.vertices[(i + 1) % mines[j].poligon.vertices.Count].x,mines[j].poligon.vertices[(i + 1) % mines[j].poligon.vertices.Count].y);
